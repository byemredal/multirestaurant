...
/**
 * `/staff/*` paths belong to the staff surface — a distinct identity with
 * its own session storage and own gate. TenantGate must NOT redirect on
 * these paths regardless of tenant-session state; the StaffSessionGate
 * inside each staff page is the routing authority there.
 */
function isStaffSurfacePath(pathname: string): boolean {
...
export function isPathAllowedForStatus(
  ...
  // Staff surface short-circuits both branches — TenantGate is a no-op there.
